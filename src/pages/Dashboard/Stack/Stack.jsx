...
function CardContent(navigate, id) {
  return (
    <div className='flex items-center gap-5'>
      <Button className='bg-green-600 w-full' type='primary'><EditFilled className='text-[20px]' /></Button>
      ...
      <ul className='flex mt-8 flex-wrap gap-5'>
        {stackList.map(item => (
          <Card
            ...
            key={item.id}
            hoverable
            cover={<img onClick={() => navigate(`${item.id}`)} className='h-[140px] object-cover' alt='example' src={item.image || "/placeholder.svg"}/>}
          >
